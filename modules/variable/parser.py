...
def failure(using_tty: bool):
    if using_tty:
        print(' '*10 + 'This line will be ignored.')
        raise InvalidInputError()
    else:
        exit_failure()


def ensure(using_tty: bool, expr: bool):
    if not expr:
        failure(using_tty)
...
def parse_character_set(using_tty: bool, name: str, characters: str) -> str:
    #    characters 'a-f0-9.#\\'
    # ->     tokens ['af' '09' '.' '#' '\']
    ...
    for i in range(len(characters)):
        ...
        if characters[i] == '\\':
            ...
            if in_character_range:
                ...
                ensure(using_tty, len(tokens) != 0)
                ensure(using_tty, len(tokens[-1]) == 1)
...
def split_two_expr(using_tty: bool, expressions: str) -> tuple[str, str]:
    paren_depth = 0
...
def parse_variable(using_tty: bool, source: typing.TextIO | io.typing.TextIOWrapper, is_verification: bool) -> tuple[
    ...
    for line in source:
        ...
        try:
            ...
            if (match_obj := re.fullmatch(int_pattern, line)) is not None:
                ...
                var.low_expr, var.high_expr = map(process_expr, split_two_expr(using_tty, match_obj[3]))
                if match_obj[2] == '(':
            ...
            # is it a floating point number?
                ...
                var.low_expr, var.high_expr = map(process_expr, split_two_expr(using_tty, match_obj[4]))
                if match_obj[3] == '(':
            ...
            # is it an array of integers?
                ...
                var.element.low_expr, var.element.high_expr = map(process_expr, split_two_expr(using_tty, match_obj[5]))
                if match_obj[4] == '(':
            ...
            # is it an array of floating point numbers?
                ...
                var.element.low_expr, var.element.high_expr = map(process_expr, split_two_expr(using_tty, match_obj[6]))
                if match_obj[5] == '(':
            ...
            # is it an array of strings?
                ...
                var.element.length_low_expr, var.element.length_high_expr = map(process_expr, split_two_expr(using_tty, match_obj[6]))
                if match_obj[5] == '(':
            ...
            # is it a matrix of integers?
                ...
                var.size_r_expr, var.size_c_expr = map(process_expr, split_two_expr(using_tty, match_obj[1]))

                var.element.low_expr, var.element.high_expr = map(process_expr, split_two_expr(using_tty, match_obj[4]))
                if match_obj[3] == '(':
                    var.element.low_expr = ['('] + var.element.low_expr + [')', '+', '1']
                if match_obj[5] == ')':
                    var.element.high_expr = ['('] + var.element.high_expr + [')', '-', '1']
            ...
            # is it a matrix of floating point numbers?
                ...
                var.size_r_expr, var.size_c_expr = map(process_expr, split_two_expr(using_tty, match_obj[2]))
            ...
            # is it a graph?
                ...
                var.name = name
                var.id = len(variables)
                var.number_of_vertices_expr, var.number_of_edges_expr = map(process_expr, split_two_expr(using_tty, match_obj[1]))
